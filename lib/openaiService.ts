...
export async function analyzeContract(
  ...
  try {
    ...
    const systemPrompt = `You are an expert Move smart contract auditor and security researcher for the Aptos blockchain.
Your task is to analyze Move modules for:
1. Security vulnerabilities (access control issues, resource safety, etc.)
2. Gas optimization opportunities
3. Code quality and best practices
4. Logic errors and potential bugs
5. Compliance with Aptos standards and best practices
...
export async function suggestTestCases(contractCode: string): Promise<AIAnalysisResult> {
  const prompt = `Suggest comprehensive test cases for this Move module. Include:
1. Unit tests for each function
2. Integration tests
3. Edge cases and boundary conditions
4. Attack vectors to test
5. Example test code using Aptos SDK and Move testing framework`;
...
export async function checkCompliance(
  contractCode: string,
  standard: string = 'Coin'
): Promise<AIAnalysisResult> {
  const prompt = `Check if this Move module properly implements Aptos ${standard} standards. Verify:
